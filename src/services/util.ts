...
function tableDifferences(tableName: string, tableColumn: string, values: string[], callback?: (additions: string[], subtractions: string[]) => void) {
...
function addToTable(tableName: string, tableColumn: string, values: string[], callback?: voidCallback) {
    if (values.length > 0) {
    ...
    mainDB.executeMany([userTable, departmentTable, conditionTable, platformTable, bookTable, passwordResetTable, verifyTable, sessionTable, reportTable, searchSortTable, metaTable], null, () => {
        ...
        setTimeout(() => {
            ...
            scrapeDepartments((err, departments) => {
                tableDifferences('Department', 'name', departments, (additions, subtractions) => {
