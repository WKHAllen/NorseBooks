...
function makeContactInfoDynamic() {
  ...
  if (platformType === "Email") {
    ...
    contactLink.setAttribute(
      "href",
      "tel:" + document.getElementById("original-contact-value").innerText
    );
...
function improveTimestamps() {
  ...
  for (var timestampDate of document.getElementsByClassName(
    "timestamp-date"
  )) {
...
function createAlert(alertValue, overrideClose) {
  if (
    alertValue &&
    (localStorage.getItem("lastAlert") !== alertValue ||
      overrideClose ||
      false)
  ) {
