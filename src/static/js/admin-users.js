...
function showOrderBy() {
	...
	if (orderDirection === 'ASC') {
		...
		sortedColumn.parent().html(sortedColumn.parent().html() + ' <i class="fas fa-caret-up"></i>');
	} else if (orderDirection === 'DESC') {
		sortedColumn.attr('href', headerHref + `&orderDirection=ASC`);
		sortedColumn.parent().html(sortedColumn.parent().html() + ' <i class="fas fa-caret-down"></i>');
	}
}
