...
export function newRandomPassword(): string {
  var examplePassword = randomPassword.randomPassword({
    length: 10,
    characters: [
...
export function replacePlaceholders(str: string, ...values: string[]): string {
  for (var value of values) str = str.replace("{}", value);
  return str;
}
...
export function sendPasswordResetEmail(email: string, hostname: string) {
  email = email.toLowerCase();
  services.PasswordResetService.newPasswordResetId(
    email,
    (passwordResetId) => {
      fs.readFile(
        ...
        (err, htmlData) => {
          ...
          fs.readFile(
            ...
            (err, textData) => {
              if (err) throw err;
              htmlData = replacePlaceholders(
                htmlData,
                hostname,
                passwordResetId
              );
              textData = replacePlaceholders(
                textData,
                hostname,
                passwordResetId
              );
...
export function validISBN(ISBN: string): boolean {
  ...
  for (var char of ISBN) if (!ISBNChars.includes(char)) return false;
  return true;
}
...
export function validBook(
  form: BookForm,
  callback: (success: boolean, error: string, book?: BookObject) => void
) {
  ...
  var price =
    Math.floor(
      parseFloat(stripWhitespace(form.price.replace("$", ""))) * 100
    ) / 100;
  ...
  if (title.length === 0 || title.length > 128) {
    callback(
      false,
      "Please enter the title of the book. It must be at most 128 characters long."
    );
  } else {
    // Check author
    if (author.length === 0 || author.length > 64) {
      callback(
        false,
        "Please enter the author's name. It must be at most 64 characters long."
      );
    } else {
      // Check department
      services.DepartmentService.validDepartment(department, (valid) => {
        if (!valid) {
          ...
          // Check course number
          if (
            stripWhitespace(form.courseNumber).length > 0 &&
            (isNaN(courseNumber) || courseNumber < 101 || courseNumber > 499)
          ) {
              ...
              services.ConditionService.validCondition(condition, (valid) => {
                if (!valid) {
                  ...
                  if (imageUrl.length > 256) {
                    callback(
                      false,
                      "Please enter a valid image URL or leave the box blank. The URL must be less than 256 characters."
                    );
                  } else {
                    // Check description
                    if (
                      ...
                      callback(
                        false,
                        "Please enter a description of at most 1024 characters."
                      );
                    } else {
                      // Check ISBN10
                      if (
                        ISBN10.length > 0 &&
                        (ISBN10.length !== 10 || !validISBN(ISBN10))
                      ) {
                        ...
                        if (
                          ISBN13.length > 0 &&
                          (ISBN13.length !== 13 || !validISBN(ISBN13))
                        ) {
...
export function logCloudinaryDestroyError(
  imageUrl: string,
  err: any,
  result: any
) {
...
export function auth(req: Request, res: Response, next: NextFunction) {
  if (!req.session || !(req.session as any).sessionId) {
    return renderPage(
      ...
      res,
      "401",
      { title: "Permission denied", after: req.originalUrl },
      401
    );
  } else {
    services.AuthService.auth((req.session as any).sessionId, (valid) => {
      if (valid) next();
      else
        return renderPage(
          req,
...
export function renderPage(
  ...
  services.MetaService.getMeta("Version", (version) => {
    ...
    if (!req.session || !(req.session as any).sessionId) {
      ...
      services.MiscService.getNavInfo(
        (req.session as any).sessionId,
        (result) => {
          if (!result) {
            options.loggedIn = false;
