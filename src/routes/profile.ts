...
import { renderPage, Request, Response, auth, stripWhitespace, formatPrice, imagePublicId, logCloudinaryDestroyError, upload, cloudinaryName, cloudinaryApiKey, cloudinaryApiSecret } from './util';
...
// Set image event
router.post('/setImage', auth, upload.single('image'), (req: Request, res: Response) => {
    services.AuthService.getAuthUser(req.session.sessionId, (userId) => {
        services.UserService.getUserImage(userId, (imageUrl) => {
            if (imageUrl) {
                cloudinary.v2.uploader.destroy(imagePublicId(imageUrl), (err, destroyResult) => {
                    logCloudinaryDestroyError(imageUrl, err, destroyResult);
                    cloudinary.v2.uploader.upload(req.file.path, (cloudinaryErr, result) => {
                        if (!cloudinaryErr) {
                            services.UserService.setUserImage(userId, result.secure_url, () => {
                                res.redirect('/profile');
                            });
                        } else {
