          ...
          if (numBooks < numMaxNumBooks) {
            services.UserService.hasContactInfo(userId, (hasInfo) => {
              if (hasInfo) {
                ...
                renderPage(req, res, "no-contact-info", {
      ...
      validBook(req.body, (valid, err, values) => {
        if (valid && !cloudinaryErr) {
          services.AuthService.getAuthUser(
            (req.session as any).sessionId,
            (userId) => {
              services.BookService.newBook(
                values.title,
                values.author,
                values.department,
                values.courseNumber || null,
                values.condition,
                values.description,
                userId,
                values.price,
                ...
                values.ISBN13 || null,
                (bookId) => {
                  (req.session as any).justListed = true;
  ...
  services.BookService.validBook(req.params.bookId, (valid) => {
    if (valid) {
      services.BookService.getBookInfo(req.params.bookId, (bookInfo) => {
        services.BookService.getUserBookInfo(
          req.params.bookId,
          (userBookInfo) => {
            services.DepartmentService.getDepartmentName(
              bookInfo.departmentid,
              (department) => {
                services.ConditionService.getConditionName(
                  bookInfo.conditionid,
                  (condition) => {
                    services.PlatformService.getPlatformName(
                      userBookInfo.contactplatformid,
                      (platform) => {
                        services.AuthService.getAuthUser(
                          (req.session as any).sessionId,
                          (userId) => {
                            services.ReportService.userReportedBook(
                              userId,
                              bookInfo.id,
                              (alreadyReported) => {
                                services.ReportService.userReportedRecently(
                                  userId,
                                  (reportedRecently) => {
                                    renderPage(req, res, "book", {
                                      ...
                                      justListed:
                                        (req.session as any).justListed ||
                                        false,
                                    });
                                    (req.session as any).justListed = undefined;
                                  }
                                );
                              }
                            );
