    ...
    services.AuthService.getAuthUser(req.session.sessionId, (userId, firstname, lastname) => {
        services.FeedbackService.canProvideFeedback(userId, (can) => {
            if (can) {
                ...
                emailer.sendEmail(emailer.emailAddress, 'User Feedback', `${firstname} ${lastname} provided the following feedback:<br><br>${feedback}<br><br>Sincerely,<br>The NorseBooks Dev Team`);
