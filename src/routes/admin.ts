            ...
            services.StatsService.getNumTables((numTables) => {
              services.StatsService.getNumRows((numRows) => {
                services.StatsService.getNumReports((numReports) => {
                  var rowsPercentage =
                    Math.floor((numRows / 10000) * 100 * 10) / 10;
                  res.json({
                    numUsers: numUsers,
                    numBooks: numBooks,
                    numSold: numSold,
...
router.post("/setBooksPerQuery", adminAuth, (req: Request, res: Response) => {
  services.MetaService.setMeta(
    "Books per query",
    req.body.booksPerQuery,
    () => {
      res.redirect("/admin");
    }
  );
...
router.get(
  ...
  (req: Request, res: Response) => {
    services.MetaService.getMeta(
      "Terms and Conditions",
      (termsAndConditions) => {
        renderPage(req, res, "admin-tac", {
    ...
    services.MetaService.getMeta("Alert timeout", (alertTimeout) => {
      if (alertValue !== null && alertTimeout !== null) {
        var remaining = Math.floor(
          parseInt(alertTimeout) - new Date().getTime() / 1000
        );
        if (remaining > 0) {
          var days = Math.floor(remaining / (60 * 60 * 24));
          var hours = Math.floor(
            (remaining - days * 60 * 60 * 24) / (60 * 60)
          );
          var minutes = Math.floor(
            (remaining - days * 60 * 60 * 24 - hours * 60 * 60) / 60
          );
          var seconds =
            remaining - days * 60 * 60 * 24 - hours * 60 * 60 - minutes * 60;
...
router.post("/alert", adminAuth, (req: Request, res: Response) => {
  ...
  if (isNaN(days) || isNaN(hours) || isNaN(minutes) || isNaN(seconds)) {
    (req.session as any).errorMsg =
      "Days, hours, minutes, and seconds must all be integers";
    res.redirect("/admin/alert");
  } else {
    if (hours < 0 || hours >= 24) {
      (req.session as any).errorMsg = "Hours must be between 0 and 23";
      res.redirect("/admin/alert");
    } else if (minutes < 0 || minutes >= 60) {
      (req.session as any).errorMsg = "Minutes must be between 0 and 59";
      res.redirect("/admin/alert");
    } else if (minutes < 0 || seconds >= 60) {
      (req.session as any).errorMsg = "Seconds must be between 0 and 59";
      res.redirect("/admin/alert");
    } else {
      var timeout =
...
router.get("/books", adminAuth, (req: Request, res: Response) => {
  ...
  services.AdminService.getBooks(orderBy, orderDirection, (books) => {
    renderPage(req, res, "admin-books", {
      books: books,
      orderBy: orderBy,
      orderDirection: orderDirection,
    });
